...
// src/binding/providers/dataset-provider.js
var DatasetProvider = class extends ProviderBase {
  constructor(element, context, property, value, ctxName, parentId) {
    super(element, context, property, value, ctxName, parentId, false);
  }
  dispose() {
...
// src/binding/providers/case-classlist-provider.js
var setValueCode = `
if (__expr__) {
    return element.classList.add(__value__);
}
`;
var setAttrCode = "return element.classList.add(__value__);";
var CaseClassListProvider = class extends ProviderBase {
  async initialize() {
    this._eventHandler = this.propertyChanged.bind(this);
    this._properties = [];
...
function createConditionsCode() {
  ...
  for (const part of parts) {
    ...
    if (subParts[0].trim() == "default") {
      code.push(setAttrCode.replace("__attr__", attr).replace("__value__", value));
      continue;
    }
    ...
    code.push(
      setValueCode.replace("__expr__", san_exp.expression).replace("__attr__", attr).replace("__value__", value)
    );
  }
...
// src/binding/providers/case-style-provider.js
var setValueCode2 = `
if (__expr__) {
    return element.__property__ = __value__;
}
`;
var setAttrCode2 = "return element.__property__ = __value__;";
var CaseStyleProvider = class extends ProviderBase {
  async initialize() {
    this._eventHandler = this.propertyChanged.bind(this);
    this._properties = [];
    createConditionsCode2.call(this);
    this.listenOnPath(this._properties, this._eventHandler);
    this.propertyChanged();
  }
  dispose() {
...
function createConditionsCode2() {
  const parts = this._value.split(",");
  const code = [];
  ...
  for (const part of parts) {
    ...
    if (subParts[0].trim() == "default") {
      hasDefault = true;
      code.push(setAttrCode2.replace("__property__", this._property).replace("__value__", value));
      continue;
    }
    const san_exp = crsbinding.expression.sanitize(subParts[0].trim(), this._ctxName);
    this._properties = [...this._properties, ...san_exp.properties];
    code.push(
      setValueCode2.replace("__expr__", san_exp.expression).replace("__property__", this._property).replace("__value__", value)
    );
  }
  if (hasDefault == false) {
    code.push(setAttrCode2.replace("__property__", this._property).replace("__value__", "'inherit'"));
  }
  const set = new Set(this._properties);
  this._properties = Array.from(set);
...
// src/binding/providers/case-attr-provider.js
var setValueCode3 = `
if (__expr__) {
    return element.setAttribute("__attr__", __value__);
}
`;
var setAttrCode3 = 'return element.setAttribute("__attr__", __value__);';
var CaseAttrProvider = class extends ProviderBase {
  async initialize() {
    this._eventHandler = this.propertyChanged.bind(this);
    this._properties = [];
    createConditionsCode3.call(this);
    this.listenOnPath(this._properties, this._eventHandler);
    this.propertyChanged();
  }
  dispose() {
...
function createConditionsCode3() {
  const parts = this._value.split(",");
  const code = [];
  let hasDefault = false;
  for (const part of parts) {
    ...
    if (subParts[0].trim() == "default") {
      hasDefault = true;
      code.push(setAttrCode3.replace("__attr__", attr).replace("__value__", value));
      continue;
    }
    const san_exp = crsbinding.expression.sanitize(subParts[0].trim(), this._ctxName);
    this._properties = [...this._properties, ...san_exp.properties];
    code.push(
      setValueCode3.replace("__expr__", san_exp.expression).replace("__attr__", attr).replace("__value__", value)
    );
  }
  if (hasDefault == false) {
...
async function parseElement(element, context, options) {
  ...
  const boundAttributes = attributes.filter(
    (attr) => attr.ownerElement.tagName.toLowerCase() == "template" && attr.name == "for" || attr.name == "ref" || attr.name.indexOf(".") != -1 || (attr.value || "").indexOf("${") == 0 || (attr.value || "").indexOf("&{") == 0
  );
...
// src/binding/providers/for-inflate-provider.js
var ForInflateProvider = class extends ProviderBase {
  dispose() {
...
var InflationCodeGenerator = class {
  ...
  async _processTranslationValue(attr) {
    const path2 = attr.value.replace("&{", "").replace("}", "");
    const value = await crsbinding.translations.get(path2);
    if (value == null) {
      this.inflateSrc.push(`crsbinding.translations.get("${path2}").then(result => ${this.path}.setAttribute("${attr.name}", result);`);
    } else {
